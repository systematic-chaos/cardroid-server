...
public class User extends UserTyp implements IUser, ObjectFactory {
	...
	public User(ObjectAdapter adapter) {
		_adapter = adapter;
	}
	 ...
	 */
	public static UserTypPrx createProxy(UserTyp userObject,
			ObjectAdapter adapter) {
		UserTypPrx userPrx = UserTypPrxHelper.uncheckedCast(adapter
				.createProxy(createIdentity(userObject.getEmail())));
		if (userObject != null)
	 ...
	 * 			servant is added to adapter 's active servant map
	 */
	public static UserTypPrx createProxy(IUser userObject, ObjectAdapter adapter) {
		UserTypPrx userPrx = UserTypPrxHelper.uncheckedCast(adapter
				.createProxy(createIdentity(userObject.getEmail())));
		if (userObject != null)
			fillProxyData(userObject, userPrx, adapter);
	 ...
	 * in the given object
	 */
	public static void fillProxyData(UserTyp object, UserTypPrx proxy,
			ObjectAdapter adapter) {
	 ...
	 * in the given object
	 */
	public static void fillProxyData(IUser object, UserTypPrx proxy,
			ObjectAdapter adapter) {
		proxy.setName(object.getName());
		proxy.setSurname(object.getSurname());
		if (object.hasAvatar()) {
			try {
				Blob avatar = object.getAvatar();
				proxy.setAvatarBytes(avatar.getBytes(1, (int) avatar.length()));
			} catch (SQLException sqle) {
				System.err.println("SQLException: " + sqle.getMessage());
			}
		}
		proxy.setUserHome(new Place().newInstance(object.getHome()));
		proxy.setTelephone(object.getTelephoneNumber());
		proxy.setEmail(object.getEmail());
		if (object.hasReputation())
			proxy.setReputation(object.getReputation());
		ICar[] objectCars = object.getCars();
		ArrayList<CarTypPrx> proxyCars = new ArrayList<CarTypPrx>(
				objectCars.length);
		for (ICar c : objectCars)
			proxyCars.add(Car.createProxy(object.getEmail(), c, adapter));
	...
	public User newInstance(IUser userObject) {
		...
		for (ICar c : userObject.getCars()) {
			userCars.add(Car.createProxy(userObject.getEmail(), c, _adapter));
		}
		...
		if (userObject.hasAvatar()) {
			try {
				userAvatar = userObject.getAvatar().getBytes(0,
	...
	public void setAvatar(Blob avatar) {
		try {
			setAvatarBytes(avatar.getBytes(0, (int) avatar.length()));
		} catch (SQLException sqle) {
			System.err.println("SQLException: " + sqle.getMessage());
		}
	}
	...
	public void setCars(ICar[] cars) {
		...
		for (ICar c : cars) {
			this.cars.add(Car.createProxy(getEmail(), c, _adapter));
		}
	}
	...
	public boolean addCar(ICar car) {
		CarTypPrx c = Car.createProxy(getEmail(), car, _adapter);
		int n;
	...
	public boolean addCar(CarTyp c, Current __current) {
		...
		for (n = 0; n < cars.size(); n++)
			if (cars.get(n).getPlate().equals(c.getPlate()))
				break;
		if (n <= cars.size())
			cars.set(n, Car.createProxy(email, c, __current.adapter));
		else
			cars.add(Car.createProxy(email, c, __current.adapter));
		return true;
	}
