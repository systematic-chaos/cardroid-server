...
public class Trip extends TripTyp implements ITrip, ObjectFactory {
	 ...
	 */
	public static TripTypPrx createProxy(TripTyp tripObject,
			ObjectAdapter adapter) {
		TripTypPrx tripPrx = TripTypPrxHelper.uncheckedCast(adapter
				.createProxy(createIdentity(tripObject.getTripId())));
		if (tripObject != null)
	 ...
	 * 			servant is added to adapter 's active servant map
	 */
	public static TripTypPrx createProxy(ITrip tripObject, ObjectAdapter adapter) {
		TripTypPrx tripPrx = TripTypPrxHelper.uncheckedCast(adapter
				.createProxy(createIdentity(tripObject.getTripId())));
		if (tripObject != null)
			fillProxyData(tripObject, tripPrx, adapter);
	 ...
	 * in the given object
	 */
	public static void fillProxyData(TripTyp object, TripTypPrx proxy,
			ObjectAdapter adapter) {
		...
		if (object.hasTripReturnDate())
			proxy.setTripReturnDate(object.getTripReturnDate());
		if (object.hasWeekDaysPeriodicity()) {
			uclm.esi.cardroid.data.zerocice.Periodicity periodicity = object
					.getTripPeriodicity();
			String[] weekDays = object.getTripWeekDays();
	 ...
	 * in the given object
	 */
	public static void fillProxyData(ITrip object, TripTypPrx proxy,
			ObjectAdapter adapter) {
		proxy.setTripId(object.getTripId());
		proxy.setPlace1(new Place().newInstance(object.getFromPlace()));
		proxy.setPlace2(new Place().newInstance(object.getToPlace()));
		proxy.setTripDate(new Date().newInstance(object.getDateTime()));
		proxy.setNSeats(object.getNSeats());
		proxy.setTripType(object.getTripType());
		if (object.hasReturnDateTime())
			proxy.setTripReturnDate(new Date().newInstance(object
					.getReturnDateTime()));
		if (object.hasWeekDaysPeriodicity()) {
			uclm.esi.cardroid.data.zerocice.Periodicity periodicity = uclm.esi.cardroid.data.zerocice.Periodicity
					.valueOf(object.getPeriodicity().name());
			String[] weekDays = null;
			char[] wd = object.getWeekDays();
