...
public class Passenger extends PassengerTyp implements IPassenger,
	...
	public Passenger(ObjectAdapter adapter) {
		_adapter = adapter;
	}
	...
	public IUser newInstance(IUser userObject) {
		return new User(_adapter).newInstance(userObject);
	}
	...
	public void setAvatar(Blob avatar) {
		...
		try {
			avatarBytes = avatar.getBytes(0, (int) avatar.length());
	...
	public void setUserPassenger(IUser passengerUser) {
		this.passengerUser = User.createProxy(passengerUser, _adapter);
	}
	...
	public void setCars(ICar[] cars) {
		...
		for (ICar ic : cars)
			c.add(Car.createProxy(passengerUser.getEmail(), ic, _adapter));
	...
	public Passenger newInstance(IPassenger passengerObject) {
		return new Passenger(User.createProxy(passengerObject, _adapter),
