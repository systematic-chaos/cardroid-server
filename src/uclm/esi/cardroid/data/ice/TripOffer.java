...
public class TripOffer extends TripOfferTyp implements ITripOffer,
	...
	public TripOffer(ObjectAdapter adapter) {
		_adapter = adapter;
	}
	...
	public TripOffer(TripOfferTyp tripOffer) {
		this(tripOffer.tripId, tripOffer.fromPlace, tripOffer.toPlace,
				tripOffer.tDateTime, tripOffer.nSeats, tripOffer.driver,
				tripOffer.waypoints, tripOffer.tripOfferCar,
	 ...
	 */
	public static TripOfferTypPrx createProxy(TripOfferTyp tOfferObject,
			ObjectAdapter adapter) {
		TripOfferTypPrx tOfferPrx = TripOfferTypPrxHelper.uncheckedCast(adapter
				.createProxy(createIdentity(tOfferObject.getTripId())));
		if (tOfferObject != null)
	 ...
	 * 			servant is added to adapter 's active servant map
	 */
	public static TripOfferTypPrx createProxy(ITripOffer tOfferObject,
			ObjectAdapter adapter) {
		TripOfferTypPrx tOfferPrx = TripOfferTypPrxHelper.uncheckedCast(adapter
				.createProxy(createIdentity(tOfferObject.getTripId())));
		if (tOfferObject != null)
	 ...
	 * in the given object
	 */
	public static void fillProxyData(TripOfferTyp object,
			TripOfferTypPrx proxy, ObjectAdapter adapter) {
	 ...
	 * in the given object
	 */
	public static void fillProxyData(ITripOffer object, TripOfferTypPrx proxy,
			ObjectAdapter adapter) {
		Trip.fillProxyData(object, proxy, adapter);
		proxy.setTripDriver(User.createProxy(object.getDriver(), adapter));
		IWaypoint[] objectWaypoints = object.getWaypoints();
		ArrayList<WaypointTyp> proxyWaypoints = new ArrayList<WaypointTyp>(
				objectWaypoints.length);
		for (IWaypoint wp : objectWaypoints)
			proxyWaypoints.add(new Waypoint().newInstance(wp));
		proxy.setTripWaypoints(proxyWaypoints);
		proxy.setTripCar(Car.createProxy(object.getDriver().getEmail(),
				object.getCar(), adapter));
		IPassenger[] objectPassengers = object.getPassengers();
		ArrayList<PassengerTyp> proxyPassengers = new ArrayList<PassengerTyp>(
				objectPassengers.length);
		Passenger passenger = new Passenger(adapter);
		for (IPassenger p : objectPassengers)
			proxyPassengers.add(passenger.newInstance(p));
		proxy.setTripPassengers(proxyPassengers);
		proxy.setPrice(object.getPrice());
		boolean[] a = object.getAllowed();
		String[] allowed = new String[a.length];
		for (int n = 0; n < a.length; n++)
			allowed[n] = String.valueOf(a[n]);
		proxy.setAllowedFeatures(allowed);
		proxy.setTripDateTime(new DateTime().newInstance(object.getDateTime()));
		if (object.hasReturnDateTime())
			proxy.setTripReturnDateTime(new DateTime().newInstance(object
					.getReturnDateTime()));
	}
	...
	public TripOffer newInstance(ITripOffer tripOfferObject) {
		...
		DateTime datetime = new DateTime().newInstance(tripOfferObject
				.getDateTime());
		UserTypPrx driver = User.createProxy(tripOfferObject.getDriver(),
				_adapter);
		...
		CarTypPrx car = Car.createProxy(tripOfferObject.getDriver().getEmail(),
		...
		ArrayList<PassengerTyp> passengers = new ArrayList<PassengerTyp>(
				p.length);
		Passenger passenger = new Passenger(_adapter);
	...
	public void setDriver(IUser driver) {
		setTripDriver(User.createProxy(driver, _adapter));
	}
	...
	public void setCar(ICar car) {
		setTripCar(Car.createProxy(driver.getEmail(), car, _adapter));
	}
	...
	public void setPassengers(IPassenger[] passengers) {
		...
		for (IPassenger ip : passengers)
			p.add(new Passenger(User.createProxy(ip, _adapter), ip.getNSeats()));
	}
	...
	public boolean addPassenger(IPassenger passenger) {
		return addTripPassenger(User.createProxy(passenger, _adapter),
				passenger.getNSeats());
	}
	...
	public DateTimeTyp getTripReturnDateTime(Current __current) {
		return hasTReturnDate() ? getTReturnDateTime() : null;
	}
